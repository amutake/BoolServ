...
Definition serialize_input (i : BoolServ.input) :=
  match i with
  | Get => serialize x00
  | Put b => serialize x01 +$+ serialize b
  end.

Definition deserialize_input : ByteListReader.t BoolServ.input :=
  tag <- deserialize;;
  match tag with
  | x00 => ByteListReader.ret Get
  | x01 => Put <$> deserialize
...
Instance input_Serializer : Serializer BoolServ.input :=
...
Section Serializers.
  ...
  Definition deserialize_output : ByteListReader.t output := Res <$> deserialize.
